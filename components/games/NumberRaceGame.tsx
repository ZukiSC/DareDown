...
const NumberRaceGame: React.FC<NumberRaceGameProps> = ({ onGameEnd, players, currentPlayerId, challenge, extraTime }) => {
  ...
  const determineLoser = useCallback((isWinner: boolean) => {
    if(isWinner) {
  ...
  useEffect(() => {
      ...
      if (timeLeft <= 0) {
          playSound('timesUp');
          onGameEnd(determineLoser(false));
          return;
      }
  ...
  }, [timeLeft, currentNumber, onGameEnd, determineLoser]);
